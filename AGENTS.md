...
- `media/ugtbrowser_image_translation.png`: high-color APNG animated in-place image translation flow. Use one fixed crop for every frame; frame-to-frame x/y drift looks bad and should be treated as a failed capture. The README no longer embeds this APNG (it uses the static `ugtbrowser_image_translation_compare.png` still instead); the APNG is retained as the source-of-truth animation that the social MP4 is built from.
...
- `media/ugtbrowser_image_translation_compare.png`: static before/after still for the Chrome Web Store and README. It crops the same fixed photo region out of the two real capture frames that feed the social MP4 (`frames-complex-real/social-mp4-first.png` original and `social-mp4-final.png` translated) and lays them side by side with a red arrow. It must use the real extension output, never the synthetic painted `demo-original.png`/`demo-translated.png` fixtures. Regenerate with `python local/readme-capture/generate_image_translation_compare.py`; if the source frames change size, re-check the `PHOTO_CROP` box in that script.
